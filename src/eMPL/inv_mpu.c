...
#define i2c_write   I2C_writeMulti
#define i2c_read    I2C_readMulti
#define delay_ms    osDelay
#define min(a,b) ((a<b)?a:b)
/* Get current timestamp.
*  @param[out] timestamp: note that this value is time in 1/10000 second.
*/
void get_ms(unsigned long *timestamp){
	*timestamp = getCurrentCount_Timer1();
}
...
int mpu_dmp_get_data_raw(short *gyro, short *accel, long *quat,
    unsigned long *timestamp, short *sensors)
{
	unsigned char more;
	return dmp_read_fifo(gyro, accel, quat, timestamp, sensors, &more);
}
...
//返回值:0,正常
//    其他,失败
uint8_t mpu_dmp_get_data(float *q_0,float *q_1,float *q_2,float *q_3,float *pitch,float *roll,float *yaw)
{
